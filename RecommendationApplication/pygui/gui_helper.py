...
# Convert a list of recommendation to a table for choice 1
def plot_choice1table(root, recommendation):
    subframe_message = tk.Frame(root)
    # Show the message
    message = tk.Label(master=subframe_message,
                       text='Here are our recommendation sort by flavour similarity:')
    message.pack()
    subframe_message.pack()

    # Loading take time which is noticeable
    subframe_wait = tk.Frame(subframe_message)
    wait_message = tk.Label(master=subframe_wait,
                            text='Please wait...')
    wait_message.pack()
    subframe_wait.pack()
    ...
    for i in range(len(recommendation)):
        for j in range(2):
            if j == 0:
                ...
                e = Entry(subframe_recommention, relief=RIDGE)
                e.grid(row=i, column=j, sticky=NSEW)
                e.insert(END, recommendation[i])

    subframe_recommention.pack()
    clear_frame(subframe_wait)

# Convert a list of recommendation to a table for choice 2
def plot_choice2table(root, recommendation):
...
def display_frontpage(whiskydata, whiskynames, whisky2group):
    ...
    def frontpage_selection(event):
        ...
        def choice1_button_response():
            ...
            # Validiate user's input
            validname, recommendation = similar_recommendation(whiskydata,
                                                               user_input, whiskynames,
                                                               whisky2group, whisky_features[:-1])

            if validname:
                plot_choice1table(frame3, recommendation)
            else:
                message = f"We cannot find this in our database, "
                message += f"please enter another one!"
                messagebox.showinfo(
                    title='Error!',
                    message=message
                )
        ...
        def choice2_dropdown(event):
            ...
            if choice == 'I still cannot choose!':
                ...
                def flavour_choice(event):
                    ...
                    recommendation = flavour_recommendation(whiskydata, choice_flavour,
                                                            num_choice_strength, whisky_features,
                                                            strengthlist)
                    successful_recommendation = plot_choice2table(frame_recommendation,
                                                                  recommendation)
                ...
                question3 = tk.Label(master=frame3_question,
                                     text='Please select a flavour:')
                question3.pack()
        ...
        if choice == 1:
            ...
            frame2_button = tk.Button(frame2_button_frame, text='Submit',
                                      command=choice1_button_response)
            frame2_button.pack()
        ...
        if choice == 2:
            # Show instruction
            text = 'Please select your preferred flavour in the following options:'
            headline_secondchoice.config(text=text)
            # Layout all GUI objects
            frame2_dropdown_frame = tk.Frame(frame_page2_input)
            frame3 = tk.Frame(frame_page2_input)
            frame2_dropdown = create_dropdown(
                frame2_dropdown_frame, whisky_features)
            # Bind and Pack the changes
    ...
    frontpage_dropdown_menu = ['1 - Start by entering whisky name',
                               '2 - Start by choosing flavor',
